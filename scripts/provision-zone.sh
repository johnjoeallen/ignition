...
#
#   BASE_DOMAIN=ignition.example ./scripts/provision-zone.sh <slug> [--node <name>] [--label <l>]
#
...
: "${BASE_DOMAIN:?set BASE_DOMAIN (e.g. ignition.example)}"
...
if [ -z "$NODE" ]; then
    ...
    node_exists "$NODE" || die "no such node: $NODE (see: ign node list)"
fi
DH="$(node_get "$NODE" DOCKER_HOST)"
GIT_HOST="$(git_host "$SLUG")"
ZADMIN_HOST="$(zadmin_host "$SLUG")"
...
APP_PORT=$APP_PORT
GIT_HOST=$GIT_HOST
ZADMIN_HOST=$ZADMIN_HOST
REGISTRY=$GIT_HOST
FORGEJO_URL=https://$GIT_HOST/
ZADMIN_URL=https://$ZADMIN_HOST/
APPS_BASE=apps.$SLUG.$BASE_DOMAIN
EOF

# Traefik file-provider snippet on the control host: admin.<slug>.<base> -> ign-control.
mkdir -p "$STATE_DIR/control/dynamic"
: "${IGN_CONTROL_PORT:=8790}"
if [ ! -f "$STATE_DIR/control/dynamic/_platform.yml" ]; then
    cat > "$STATE_DIR/control/dynamic/_platform.yml" <<EOF
# generated by provision-zone.sh — routes admin.$BASE_DOMAIN to ign-control
http:
  routers:
    ign-platform:
  ...
  services:
    ign-control:
      loadBalancer:
        servers:
          - url: "http://host.docker.internal:$IGN_CONTROL_PORT"
EOF
fi
cat > "$STATE_DIR/control/dynamic/$SLUG.yml" <<EOF
# generated by provision-zone.sh — routes $ZADMIN_HOST to ign-control
http:
  routers:
    ign-zone-$SLUG:
      rule: "Host(\`$ZADMIN_HOST\`)"
      entryPoints: ["websecure"]
      service: ign-control
      tls:
        certResolver: le
        domains:
          - main: "$SLUG.$BASE_DOMAIN"
            sans: ["*.$SLUG.$BASE_DOMAIN"]
EOF
...
echo "==> zone=$SLUG  node=$NODE ('${DH:-local}')  forgejo=https://$GIT_HOST/  apps=*.apps.$SLUG.$BASE_DOMAIN"
...
cat <<EOF
  ...
  provisioned zone: $SLUG   (node $NODE)
    ...
    Forgejo service     state/zones/$SLUG/zone-admin.txt          (ign-control's - keep on the control host, do NOT hand out)

  DNS:
    $GIT_HOST  and  *.apps.$SLUG.$BASE_DOMAIN   -> node $NODE's host
    $ZADMIN_HOST                                -> the control host
  A single *.$SLUG.$BASE_DOMAIN record covers git + apps when the zone shares
  the control host; otherwise split it as above. ign control must be running so
  its Traefik picks up state/control/dynamic/$SLUG.yml.
EOF
