...
# Traefik file-provider snippet on the control host: admin.<slug>.<base> -> hz-control.
mkdir -p "$STATE_DIR/control/dynamic"
: "${HZ_CONTROL_PORT:=8790}"
if [ ! -f "$STATE_DIR/control/dynamic/_platform.yml" ]; then
    cat > "$STATE_DIR/control/dynamic/_platform.yml" <<EOF
# generated by provision-zone.sh — routes admin.$BASE_DOMAIN to hz-control
http:
  routers:
    hz-platform:
  ...
  services:
    hz-control:
      loadBalancer:
        servers:
          - url: "http://host.docker.internal:$HZ_CONTROL_PORT"
EOF
fi
cat > "$STATE_DIR/control/dynamic/$SLUG.yml" <<EOF
# generated by provision-zone.sh — routes $ZADMIN_HOST to hz-control
http:
  routers:
    hz-zone-$SLUG:
...
cat <<EOF
  ...
  the control host; otherwise split it as above. hz control must be running so
